...
//            System.out.println("commMatrix: "+commMatrix.length+" "+commMatrix[0].length);
//            System.out.println("execMatrix: "+execMatrix.length+" "+execMatrix[0].length);
...
//        mapping = PSOSchedularInstance.run();
        ...
        //======================================
        double[][] etc = new double[execMatrix.length][execMatrix[0].length];
        mapping = new double[execMatrix.length];
        for(int i=0;i<commMatrix.length;i++) {
        	for(int j=0;j<commMatrix[0].length;j++) {
        		etc[i][j]=commMatrix[i][j]+execMatrix[i][j];
        	}
        }
        int[][] task_vm = MAMIN.MAX_MIN(etc, etc[0].length);
//        int[][] task_vm = MMIN.MIN_MIN(etc, etc[0].length, false);
//        System.out.println(task_vm.length);
        ...
        try {
            ...
            Log.printLine(MAXMIN_Scheduler.class.getName() + " finished!");
        } catch (Exception e) {
            e.printStackTrace();
